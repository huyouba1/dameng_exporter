...
| 字段名 | 含义 | 返回值说明 |
|--------|------|------------|
| ARCH_STATUS | 归档状态 | 0=INVALID（无效），1=VALID（有效），2=ASYNC_SEND（异步发送），-2=UNKNOWN（未知），-3=EMPTY_OR_NULL（空值/NULL）；`dmdbms_arch_status` 在未启用归档时返回 -1 |
...
Exporter 会额外暴露 `dmdbms_arch_send_last_code` 指标；若数据库版本缺少 `LAST_SEND_CODE` 字段，则该指标不输出。
